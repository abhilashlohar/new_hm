<?php foreach($result_import_record as $data_import){
	...
	$step5=(int)@$data_import["import_record_petty_payment"]["step5"];
	$date=@$data_import["import_record_petty_payment"]["date"];
	$file_name=@$data_import["import_record_petty_payment"]["file_name"];
}
...
<div class="portlet box green" style="width: 50%; margin: auto;">
	<div class="portlet-title">
		<h4><i class="icon-cogs"></i> Import Petty Cash Payment</h4>
	</div>
	<div class="portlet-body" align="">
		<form method="post" id="form1" style="margin: 0px;">
			...
			<a href="<?php echo $webroot_path; ?>Cashbanks/petty_cash_payment_sample"><b>Click here for sample format</b></a><br/><br/>
			<h4>Instruction set to import </h4>
			<ol>
			<!--<li>Bank Receipt import Receipt mode type field can be take cheque/neft/pg </li>-->
...
<div style="width: 40%; margin: auto; background-color: rgb(210, 243, 196); border: 2px solid rgb(113, 177, 85); padding: 10px;">
	...
	<br/><br/>
	<a href="<?php echo $webroot_path; ?>Cashbanks/modify_petty_cash_csv_data" class="btn red"  id="pulsate-regular">MODIFY DATA</a>
</div>
<?php } ?>
...
$('form#form1').submit( function(ev){
	...
	if(allow=="yes"){
		...
		$.ajax({
		url: "Upload_petty_cash_payment_csv_file",
		data: m_data,
		processData: false,
		contentType: false,
		type: 'POST',
