							...
							$result_income_head = $this->requestAction(array('controller' => 'hms', 'action' => 'ledger_account_fetch2'),array('pass'=>array($key)));	
								...
								if(!empty($value)){
									$bill_html.='<tr>
										...
										<td align="center" style="border-right: solid 1px #767575;padding: 0 0 0 5px;">';
										if(!empty($income_head_for_rate[$key])){   $bill_html.=''.number_format($income_head_for_rate[$key],2).''; } $bill_html.='</td>
