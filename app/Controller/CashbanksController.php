...
function import_petty_cash_payment_csv(){
	...
	if(@$process_status==4){
		...
		$conditions=array("society_id" => $s_society_id,"is_imported" => "YES");
		$total_converted_records = $this->petty_cash_csv_converted->find('count',array('conditions'=>$conditions));
		
		$this->loadmodel('petty_cash_csv_converted');
		$conditions=array("society_id" => $s_society_id);
		$total_records = $this->petty_cash_csv_converted->find('count',array('conditions'=>$conditions));
...
function modify_petty_cash_csv_data($page=null){
	if($this->RequestHandler->isAjax()){
	$this->layout='blank';
	}else{
	$this->layout='session';
	...
	if($process_status==3){
		...
		$this->loadmodel('petty_cash_csv_converted'); 
		$conditions=array("society_id"=>(int)$s_society_id);
		$count_bank_receipt_converted=$this->petty_cash_csv_converted->find('count',array('conditions'=>$conditions));
		$this->set('count_bank_receipt_converted',$count_bank_receipt_converted);
	}
	
}

function party_account_change_by_acgroup($account_type=null,$auto_id=null,$sundry_creditor=null,$type=null){
	...
	if($account_type==1){
			...
			?>
		<select class="" record_id="<?php echo $auto_id; ?>" field="party_account"> 
		    <option> </option>
			 <?php 	
			 foreach($result_ledger_sub_account as $data){
		 ...
		 <?php
			...
			$result_ac_group=$this->accounts_group->find('all',array('conditions'=>$conditions));
			foreach($result_ac_group as $collection){
				$accounts_group_id = (int)$collection['accounts_group']['auto_id'];
				$result_ledger_account = $this->requestAction(array('controller' => 'hms', 'action' => 'expense_tracker_fetch2'),array('pass'=>array($accounts_group_id)));
				foreach($result_ledger_account as $collection2){
					$ledger_id = (int)$collection2['ledger_account']['auto_id'];
					$name = $collection2['ledger_account']['ledger_name'];
				?>
				<option value="<?php echo $ledger_id; ?>" <?php if($ledger_id==$sundry_creditor){?> selected <?php } ?> ><?php echo $name; ?></option>
		...
		?>
		<select class="m-wrap medium" name="tax[]" record_id="<?php echo $auto_id; ?>" field="party_account">
			<option value="" style="display:none;">Select</option>
			<option value="16" <?php if($sundry_creditor==16){ ?> selected <?php } ?> >Tax deducted at source (TDS payable)</option>
		</select>
...
function auto_save_petty_cash_payment($auto_save_id=null,$field=null,$value=null){

  $this->layout=null;
	
	$this->ath();
	$s_society_id = $this->Session->read('society_id');
	$record_id=(int)$auto_save_id; 
	
	if($field=="trajection_date"){
		
			$this->loadmodel('petty_cash_csv_converted');
			$this->petty_cash_csv_converted->updateAll(array("trajection_date" => $value),array("auto_id" => $record_id));
	...
	if($field=="amount"){ 
		    ...
		    $this->loadmodel('petty_cash_csv_converted');
			$this->petty_cash_csv_converted->updateAll(array("amount" => $value),array("auto_id" => $record_id));
	...
	if($field=="narration"){
			...
			$this->loadmodel('petty_cash_csv_converted');
			$this->petty_cash_csv_converted->updateAll(array("narration" => $value),array("auto_id" => $record_id));
...
function allow_import_petty_cash_payment(){
	...
	$this->loadmodel('import_record_petty_payment');
	$this->import_record_petty_payment->updateAll(array("step4" => 1),array("society_id" => $s_society_id, "module_name" => "PCP"));
}
...
function cancle_petty_cash_payment_import(){
	$s_society_id = $this->Session->read('hm_society_id');
	
	$this->loadmodel('petty_cash_csv_converted');
	$conditions=array("society_id"=>(int)$s_society_id);
	$this->petty_cash_csv_converted->deleteAll($conditions);
	
	$this->loadmodel('petty_cash_payment_csv');
	$conditions=array("society_id"=>(int)$s_society_id);
	$this->petty_cash_payment_csv->deleteAll($conditions);
	
	$this->loadmodel('import_record_petty_payment');
	$conditions=array("society_id"=>(int)$s_society_id);
	$this->import_record_petty_payment->deleteAll($conditions);
...
function final_import_petty_cash_payment_ajax(){
	...
	if($process_status==4){
		...
		$result_import_converted = $this->petty_cash_csv_converted->find('all',array('conditions'=>$conditions,'limit'=>2));
		
		foreach($result_import_converted as $import_converted){
					$bank_receipt_csv_id=$import_converted["petty_cash_csv_converted"]["auto_id"];
					$trajection_date=$import_converted["petty_cash_csv_converted"]["trajection_date"];
					$trajection_date = date('Y-m-d',strtotime($trajection_date));
					$trajection_date = strtotime($trajection_date); 
					$account_group_id=$import_converted["petty_cash_csv_converted"]["account_type"];
					$expense_party=$import_converted["petty_cash_csv_converted"]["sundry_creditor_id"];
					$paid_from_id=$import_converted["petty_cash_csv_converted"]["account_head"];
					$amount=$import_converted["petty_cash_csv_converted"]["amount"];
					$narration=$import_converted["petty_cash_csv_converted"]["narration"];
					...
					$this->loadmodel('cash_bank');
					$multipleRowData=Array(Array("transaction_id"=>$auto,"receipt_number"=>$i,"sundry_creditor_id"=>$expense_party,"created_on"=>$current_date,"account_type"=>$account_group_id,"transaction_date"=>$trajection_date,"created_by"=>$s_user_id,"narration"=>$narration,"account_head"=>$paid_from_id,"amount"=>$amount,"society_id"=>$s_society_id,"source"=>"petty_cash_payment","auto_inc"=>"YES"));
					$this->cash_bank->saveAll($multipleRowData); 
						
					if($account_group_id == 1){
						$l=$this->autoincrement('ledger','auto_id');
						$this->loadmodel('ledger');
						$multipleRowData = Array( Array("auto_id"=>$l,"transaction_date"=>$trajection_date, "debit"=>$amount,"credit" =>null,"ledger_account_id"=>15,"ledger_sub_account_id"=>$expense_party, "table_name"=>"cash_bank","element_id"=>$auto,"society_id" => $s_society_id));
						$this->ledger->saveAll($multipleRowData);
					}else{
						$l=$this->autoincrement('ledger','auto_id');
						$this->loadmodel('ledger');
						$multipleRowData = Array( Array("auto_id"=>$l,"transaction_date"=>$trajection_date, "debit"=>$amount,"credit"=>null,"ledger_account_id"=>$expense_party,"ledger_sub_account_id" =>null,"table_name"=>"cash_bank","element_id"=>$auto,"society_id"=>$s_society_id));
						$this->ledger->saveAll($multipleRowData);
					}
					
						$l=$this->autoincrement('ledger','auto_id');
						$this->loadmodel('ledger');
						$multipleRowData = Array( Array("auto_id" => $l,"transaction_date"=>$trajection_date, "debit" => null,"credit" =>$amount,"ledger_account_id"=>$paid_from_id,"ledger_sub_account_id"=>null,"table_name"=>"cash_bank","element_id"=>$auto,"society_id"=>$s_society_id));
						$this->ledger->saveAll($multipleRowData);		
							
						$this->loadmodel('petty_cash_csv_converted');
						$this->petty_cash_csv_converted->updateAll(array("is_imported" => "YES"),array("auto_id" => $bank_receipt_csv_id));
					}
				...
				$this->petty_cash_csv_converted->deleteAll($conditions4);

				$this->loadmodel('petty_cash_payment_csv');
				$conditions4=array('society_id'=>$s_society_id);
				$this->petty_cash_payment_csv->deleteAll($conditions4);

				$this->loadmodel('import_record_petty_payment');
				$conditions4=array("society_id" => $s_society_id, "module_name" => "PCP");
				$this->import_record_petty_payment->deleteAll($conditions4);
				}else{
				 $again_call_ajax="YES"; 
				}
...
$current_date=strtotime($current_date);
$this->loadmodel('cash_bank');
$conditions=array('society_id'=>$s_society_id,"source"=>"bank_receipt",'deposite_slip'=>0,'transaction_date'=>array('$lte'=>$current_date));
$result_cash_bank=$this->cash_bank->find('all',array('conditions'=>$conditions));

foreach($result_cash_bank as $data){
	$transaction_id=(int)$data['cash_bank']['transaction_id'];
	$deposite_slip=@$data['cash_bank']['deposite_slip'];
	$this->loadmodel('cash_bank');
	$this->cash_bank->updateAll(array("deposite_slip"=>1),array("transaction_id"=>$transaction_id));
...
foreach($temp_cash_bank_datas as $data){
	...
	$auto_id=$this->autoincrement('cash_bank','transaction_id');
	$receipt_number=$this->autoincrement_with_society_ticket('cash_bank','receipt_number');
	$this->cash_bank->saveAll(Array( Array("transaction_id"=>$auto_id,"transaction_date"=>$transaction_date,"deposited_in" => $deposited_in, "receipt_mode"=>$receipt_mode, "cheque_number" => $cheque_number,"date"=>$date,"drown_in_which_bank"=>$drown_in_which_bank,"branch_of_bank"=>$branch_of_bank,"received_from"=>"residential","ledger_sub_account_id"=>$ledger_sub_account_id,"amount"=>$amount,"narration"=>$narration,"society_id"=>$s_society_id,"created_by"=>$created_by,"source"=>"bank_receipt","applied"=>"no","receipt_number"=>$receipt_number,"bill_reference"=>@$bill_reference,"created_on"=>$created_on,"approved_by"=>$s_user_id,"approved_on"=>$approved_on,'deposite_slip'=>0))); 
	//$receipt_array_num[]=$receipt_number;

$this->loadmodel('ledger');
$ledger_id=$this->autoincrement('ledger','auto_id');
$this->ledger->saveAll(Array( Array("auto_id"=>$ledger_id,"transaction_date"=> $transaction_date,"debit" => $amount,"credit"=>null,"ledger_account_id"=>33, "ledger_sub_account_id" => $deposited_in,"table_name"=>"cash_bank","element_id"=>$auto_id, "society_id"=>$s_society_id))); 
	
$ledger_id=$this->autoincrement('ledger','auto_id');
$this->ledger->saveAll(Array( Array("auto_id"=>$ledger_id,"transaction_date"=>$transaction_date,"credit"=>$amount,"debit"=>null,"ledger_account_id"=>34,"ledger_sub_account_id" => $ledger_sub_account_id,"table_name"=>"cash_bank","element_id"=>$auto_id,"society_id" => $s_society_id)));
