		...
		<table class="table table-condensed table-bordered table-striped table-hover" id="main">
			...
			<tbody>
				<?php foreach($regular_bills as $data){
					...
					$member_info_created_by = $this->requestAction(array('controller' => 'Fns', 'action' => 'member_info_via_ledger_sub_account_id'),array('pass'=>array($created_by)));
					$current_date=$data["regular_bill"]["current_date"];
					$edit_text=@$data["regular_bill"]["edit_text"];
					if(!empty($edit_text)){
						$edited_by=$data["regular_bill"]["edited_by"];
						$edited_on=$data["regular_bill"]["edited_on"];
						$member_info_edit = $this->requestAction(array('controller' => 'Fns', 'action' => 'member_info_via_ledger_sub_account_id'),array('pass'=>array($edited_by)));
						$edit_info="<br/><b>Edited By: </b>".$member_info_edit["user_name"]." ".$member_info_edit["wing_name"].'-'.$member_info_edit["flat_name"]."<br/><b>Edited On: </b>".date("d-m-Y",strtotime($edited_on));
					}else{$edit_info="";}?>
					<tr>
						...
						<td>
							...
							<a href="#" class="btn mini black popovers" data-trigger="hover" data-placement="left" data-content="<b>Generated By: </b><?php echo $member_info_created_by["user_name"]." ".$member_info_created_by["wing_name"].'-'.$member_info_created_by["flat_name"]; ?><br/><b>Generated On: </b><?php echo date("d-m-Y",strtotime($current_date)); ?><?php echo $edit_info; ?>" role="button"><i class="icon-exclamation-sign"></i></a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
