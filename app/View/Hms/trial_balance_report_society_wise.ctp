<?php foreach($result_import_record as $data_import){
	...
	$step5=(int)@$data_import["trial_balance_report"]["step5"];
	$date=@$data_import["trial_balance_report"]["date"];
...
<div style="width: 40%; margin: auto; background-color: rgb(210, 243, 196); border: 2px solid rgb(113, 177, 85); padding: 10px;">
	...
	<br/><br/>
	<a href="<?php echo $webroot_path; ?>Hms/trial_balance_report_modify" class="btn red"  id="pulsate-regular">MODIFY DATA</a>
</div>
<script>
$( document ).ready(function() {
	change_page_automatically("<?php echo $webroot_path; ?>Hms/trial_balance_report_modify");
});
</script>
<?php } ?>
