...
if(!empty($move_by_id)){
			$creator_info=$this->requestAction(array('controller'=>'Fns','action'=> 'member_info_via_user_id'),array('pass'=>array((int)$move_by_id)));
			$move_by=$creator_info["user_name"];
			$wing_flats=$creator_info['wing_flat'];
			foreach($wing_flats as $wing_flat){

			}
			$move_on_date=date('d-m-Y',strtotime($move_on_date));
			$edit_info="<br/><b>Moved By: </b>".$move_by." ".$wing_flat."<br/><b>Moved On: </b>".$move_on_date."";	
		}else{
			$edit_info='';
		 }
