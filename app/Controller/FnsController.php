...
function calculate_arrears_and_interest($ledger_sub_account_id,$start_date){
	...
	foreach($result_ledger as $transaction){
		...
		$intrest_on_arrears+=($last_bill_maint_arrear*$days*$tax_factor)/365;
		
		if($current_transaction_date>$last_due_date && $bill_count>0){
			$days=abs(floor(($last_due_date-$current_transaction_date)/(60*60*24)));
			$intrest_on_arrears+=($last_bill_amount*$days*$tax_factor)/365;
		...
		if($table_name=="opening_balance"){
			...
			if(empty($debit) && !empty($credit)){
				...
				$last_bill_arrear_intrest=$arrear_intrest;
		...
		return array("maint_arrear"=>$last_bill_maint_arrear,"non_maint_arrear"=>$last_bill_non_maint_arrear,"bill_amount"=>$last_bill_amount,"arrear_intrest"=>$last_bill_arrear_intrest,"intrest_on_arrears"=>$last_bill_intrest_on_arrears);
}
