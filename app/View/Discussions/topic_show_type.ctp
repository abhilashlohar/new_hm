...
<?php
if($type_list=="my"){ ?>
<div align="center" style="color: rgb(84, 83, 83); font-weight: 600; width:100%;" >My Topics</div>
		<?php 
		if(empty($posts)){ echo'<center>No any record found </center>';  }
		foreach($posts as $post){
			$discussion_post_id=$post["discussion_post"]["discussion_post_id"];
			...
			$result_count_comment=$this->requestAction(array('controller' => 'Discussions', 'action' => 'count_comment_via_discussion_post_id'), array('pass' => array($discussion_post_id)));
			$time=$post["discussion_post"]["time"];?>
			<span class="btn mini pull-right move_archive tooltips" data-placement="top" post_id="<?php echo $discussion_post_id; ?>" style="margin-left:10px;" data-original-title="close topics" >
...
<div align="center" style="color: rgb(84, 83, 83); font-weight: 600; width:100%;">Archive Topics</div>
		<?php 
		if(empty($posts)){ echo'<center>No any record found </center>';  }
		foreach($posts as $post){
			$discussion_post_id=$post["discussion_post"]["discussion_post_id"];
			...
			$time=$post["discussion_post"]["time"];?>
			<div class="show_list" style="width:100%;">
				<span class="btn mini pull-right delete_per" style="margin-right: 0px;" post_id="<?php echo $discussion_post_id; ?>">
				<i class="icon-trash"></i>
				</span>
